...
def parse_args():
    ...
    parser.add_argument(
        "--spatial", required=True, help="Path to spatial transcriptomics h5ad file"
    )
    ...
    parser.add_argument(
        "--counts-layer",
        default=None,
        help="Layer in spatial_adata with raw counts (default: use .X), "
        "-- DOT expects raw counts for the spatial data.",
    )
    parser.add_argument(
        "--ref-counts-layer",
        default=None,
        help="Layer in ref_adata with raw counts (default: use .X). "
        "-- DOT expects raw counts for the reference.",
    )

    # Output
    parser.add_argument(
        "--output", "-o", default="dot_results", help="Output prefix for result files"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--max-genes", type=int, default=5000, help="Maximum number of genes to use"
    )
    parser.add_argument(
        "--th-spatial",
        type=float,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.01,
        help="Minimum fraction of spots a spatial gene must be expressed in "
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.99,
        help="Maximum fraction of spots a spatial gene must be expressed in "
        "(1 disables the upper bound)",
    )
    parser.add_argument(
        "--batch-size", type=int, default=5000, help="Batch size for GPU processing"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--resume-from", default=None, help="Path to a checkpoint file to resume from"
    )

    # Flags
    parser.add_argument("--no-plots", action="store_true", help="Skip generating plots")
    parser.add_argument(
        "--no-h5ad", action="store_true", help="Skip saving h5ad files per sample"
    )
    parser.add_argument(
...
def main():
    ...
    if args.counts_layer is not None and args.counts_layer != "X":
    ...
    if args.ref_counts_layer is not None and args.ref_counts_layer != "X":
        if args.ref_counts_layer in ref_adata.layers:
            ...
            print(f"  Warning: Layer '{args.ref_counts_layer}' not found, using .X")
    ...
    if args.lineage_key is not None and args.lineage_key in ref_adata.obs.columns:
        lin_dict = (
            ref_adata.obs[[args.cell_type_key, args.lineage_key]]
    ...
    for sample_id in samples:
        ...
        spatial_processed = setup_spatial(
            ...
            th_gene_low=args.th_gene_low,
            th_gene_high=args.th_gene_high,
            radius="auto",
            verbose=args.verbose,
        )
        ...
        # Run DOT
        dot = DOT(
            spatial_processed, ref_processed, batch_size=args.batch_size, device=device
        )
        ...
        weights_df = pd.DataFrame(weights, index=sample_indices, columns=cell_types)
        ...
        if not args.no_plots:
            try:
                ...
                weights_plot_path = (
                    figures_dir / f"{args.output}{sample_suffix}_weights.png"
                )
                ...
                history_plot_path = (
                    figures_dir / f"{args.output}{sample_suffix}_convergence.png"
                )
        ...
        spatial_adata.obs.loc[sample_indices, "cell_type"] = sample_data.obs[
            "cell_type"
        ].astype(str)
        if "lineage" in sample_data.obs.columns:
            spatial_adata.obs.loc[sample_indices, "lineage"] = sample_data.obs[
                "lineage"
            ].astype(str)
    ...
    spatial_adata.obs["cell_type"] = pd.Categorical(
        spatial_adata.obs["cell_type"], categories=all_cell_types, ordered=True
    )
    ...
    if "lineage" in spatial_adata.obs.columns:
        ...
        spatial_adata.obs["lineage"] = pd.Categorical(
            spatial_adata.obs["lineage"], categories=all_lineages, ordered=True
        )
    ...
    if args.save_combined:
        ...
        for sample_id in samples:
            sample_suffix = f"_{sample_id}".replace(" ", "_").replace("/", "_")
            sample_weights_path = (
                output_dir / f"{args.output}{sample_suffix}_weights.csv"
            )
